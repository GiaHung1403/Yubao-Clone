...
import { TGetCustomEmoji } from '@models/types';
...
class Markdown extends PureComponent<IMarkdownProps, any> {
	...
	renderParagraph = ({ children }: any) => {
		...
		return (
			<Text
				style={[styles.text, style, this.props.styles]}
	...
	renderEditedIndicator = () => {
		...
		return (
			<Text style={[styles.edited, { color: '#9ca2a8' }]}>
				{' '}
				({I18n.t('edited')})
			</Text>
		);
	...
	renderHeading = ({ children, level }: any) => {
		...
		return (
			<Text numberOfLines={numberOfLines} style={[textStyle, { color: '#2f343d' }]}>
				{children}
			</Text>
		);
	};
