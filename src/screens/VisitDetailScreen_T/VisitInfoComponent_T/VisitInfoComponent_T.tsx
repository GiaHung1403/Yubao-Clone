...
export default function VisitInfoComponent_T(props: any) {
	...
	const [travelTime, setTravelTime] = useState(visitInfo?.trvL_TIME.toString());
	...
	const _onPressUpdate = async () => {
		...
		try {
			const response = (await updateVisit_Trading({
				User_ID: dataUserSystem.EMP_NO,
								...
								: 'SellerListScreen',
							{
								idCustomerExisted:
									'LS_NM' in customerSelected
										...
										? customerSelected?.SELLER_NM
										: 'new',
								screenBack: 'VisitDetailScreen_T',
							},
						)
					}
				/>
			...
			<Card style={{ padding: 8, marginTop: 8 }}>
				...
				<View style={{ flexDirection: 'row', marginBottom: 12 }}>
					...
					<TextInputCustomComponent
						...
						placeholder=""
						style={{ flex: 1 }}
						value={'SRC_NM' in customerSelected ? customerSelected?.SRC_NM : ''}
						enable={false}
					/>
				</View>
