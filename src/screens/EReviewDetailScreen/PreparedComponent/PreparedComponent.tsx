...
export default function PreparedComponent(props: IProps) {
	...
	const source = `<p>${eReviewItem?.rmks}</p>`;
			...
			<View style={{}}>
				...
				<View style={{ flexDirection: 'row', marginBottom: 8 }}>
					...
					<View style={styles.containerViewStatus}>
						{eReviewItem.functioN_NAME === 'PCR' ? (
							<RenderHTMLComponent value={eReviewItem.status} stylesheet={{}} />
						) : (
							<Text>{eReviewItem.status}</Text>
						)}
						<View
							style={[
								styles.circleStatus,
								{ backgroundColor: getColorStatus(eReviewItem.status) },
							]}
						/>
					</View>
				</View>
