...
import {
    InteractionManager, Platform, SafeAreaView, StatusBar,
...
const listSortOption = [
    {id: 'alphabetic', label: 'Alphabetical', value: 'alphabetic', icon: 'swap-vertical-outline'},
    {id: 'activity', label: 'Activity', value: 'activity', icon: 'time-outline'},
];

export default function SortRoomModal(props: any) {
    ...
    const renderItemSort = ({label, icon, isSelected, onSelected}) => (
        <TouchableOpacity
            style={{borderBottomColor: '#ddd', borderBottomWidth: 1, paddingVertical: 16}}
            onPress={onSelected}
        >
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
                <Icon as={Ionicons} name={icon} size={6}/>
                <Text style={{flex: 1, marginLeft: 16}}>{label}</Text>
                {isSelected && <Icon as={Ionicons} name={'checkmark-outline'} size={6} color={colors.primary}/>}
            </View>
        </TouchableOpacity>
    );
        ...
        <View style={{flex: 1}}>
            ...
            {doneLoadAnimated && (
                <View style={{flex: 1, padding: 8}}>
                    ...
                    {listSortOption.map(sortOption => renderItemSort({
                        label: sortOption.label,
                        icon: sortOption.icon,
                        isSelected: sortOption.id === sortOptionSelected?.sortOptionSelected,
                        onSelected: () => setSortOptionSelected(oldOption => ({
                            ...oldOption,
                            sortOptionSelected: sortOption.id,
                        })),
                    }))}

                    <Text style={{fontSize: 15, fontWeight: '600', marginTop: 20}}>Group by</Text>
                    ...
                    <Button mode={'contained'} uppercase={false} style={{marginTop: 20}} onPress={_onPressSort}>
                        Sort
                    </Button>
                </View>
            )}
