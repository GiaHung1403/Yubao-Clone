...
import {
	getListRequestEFlow,
	setDescriptionEFlow,
	setKindSelectedEFlow,
	setLesseeNameEFlow,
	setProposedByEFlow,
	setRequestIDEFlow,
	setStatusSelectedEFlow,
...
export default function EFlowFilterComponent(props: any) {
	...
	const _onPressFilter = async () => {
		...
		dispatch(
			getListRequestEFlow({
				User_ID: dataUserSystem.EMP_NO, //00221
				Status: statusSelected,
				Kind: kindSelected,
				RequestID: requestID,
				ProposedBy: proposedBy,
		...
		<Card elevation={4}>
			...
			>
				<PickerCustomComponent
					...
					onValueChange={text =>
						dispatch(setKindSelectedEFlow({ kindSelected: text }))
					}
				/>
				...
				<TextInputCustomComponent
					...
					onChangeText={(text: string) =>
						dispatch(setRequestIDEFlow({ requestID: text }))
					}
					style={{ flex: 1 }}
				/>
			</View>
			...
			>
				<TextInputCustomComponent
					...
					onChangeText={(text: string) =>
						dispatch(setLesseeNameEFlow({ lesseeName: text }))
					}
					style={{ flex: 1, marginRight: 8 }}
				/>
				...
				<PickerCustomComponent
					...
					onValueChange={text =>
						dispatch(setStatusSelectedEFlow({ statusSelected: text }))
					}
				/>
			</View>
			<Animated.View
				...
				{showView && (
					...
					>
						<TextInputCustomComponent
							...
							keyboardType="number-pad"
							onChangeText={(text: string) =>
								dispatch(setDescriptionEFlow({ description: text }))
							}
							style={{ flex: 1, marginRight: 8 }}
						/>
						...
						<TextInputCustomComponent
							...
							value={proposedBy}
							onChangeText={(text: string) =>
								dispatch(setProposedByEFlow({ proposedBy: text }))
							}
							style={{ flex: 1 }}
						/>
					</View>
