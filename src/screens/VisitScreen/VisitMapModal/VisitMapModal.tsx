...
import { getAddressVisit, insertCheckinVisit } from '@data/api';
...
export function VisitMapModal(props: any) {
	...
	const _onPressCheckIn = async () => {
		try {
			...
			await insertCheckinVisit({
				leseID: visitInfo.lese_ID,
				location: `${currentLocation?.latitude},${currentLocation?.longitude}`,
				visitID: visitInfo.seq,
				VisitChecked_YN:
					getDistanceFromLocation(placeLocation) < visitInfo.distance,
				...
				>
					{placeLocation && (
						<Circle
							...
							radius={Number(visitInfo?.distance) || 0}
